...
public class PrintTableServiceImpl implements PrintTableService{
    ...
    public PageBean<Goods> byPage(DividePageParameter dividePageParameter) {
        ...
        pb.setRows(rows);
        //调用dao查询总记录数
        int totalCount = factory.getPrintDao().findTotalCount(goods);
        pb.setTotalCount(totalCount);
        //调用list集合
        //计算开始记录的索引
        int start = (currentPage - 1) *rows;
        List<Goods> list = factory.getPrintDao().findByPage(start,rows,rangeMin,rangeMax,rank,goods);
        pb.setList(list);
        //计算总页码
    ...
    public List<User> selectPersonalMessage(String username){
        Factory factory = new Factory();
        return factory.getPrintDao().selectPersonalMessage(username);
    }
}